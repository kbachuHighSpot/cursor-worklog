...
- Opened the deferred index-drop follow-up as a **Draft PR** (#71199) on branch `HS-155824/drop-stale-email-tracking-details-indexes` (worktree at `/Users/kiran.bachu/Codebase/nutella-HS-155824-drop-old-indexes`). Drops `[details:1, created_at:-1]` (mongo_a) and `[details:1, created_at:1]` (mongo_e). Body includes a DO NOT MERGE banner, a three-step Atlas verification checklist (`getIndexes()` for the new partial index, `explain("executionStats").updateMany()` to confirm the planner picks `for_email_tracking_details_v1`, and a New Relic / runtime sanity check on `email_tracking_details_v1`), and a rollback note. Cross-referenced in #71197's body and a tracking comment.
- Cross-checked the fix against actual Atlas Insights data from a real timed-out run (`Sat 01/24/26 09:14:38`, host `production-shard-00-01.sp8ho.mongodb.net`, 2.20 hr execution, 125,485,900 keys/docs examined, 1 doc returned, `Has Sort Stage: No`). The data shape revealed the planner actually picked the plain `[created_at: 1]` index — not the wrong-direction `[details:1, created_at:-1]` compound I'd originally hypothesized — and treated `details: $exists` as a per-FETCH residual predicate. Updated #71197 description with an "Evidence from Atlas Insights" section containing the full slow query JSON, a stats table, the corrected failure-mode analysis, and the relevant stack-trace excerpt; also corrected the embedded code snippet to use `$exists => true` (boolean) consistent with the committed code. The fix still resolves the issue either way — the `find().sort().limit(1)` query is structurally gone in the rewrite — but the corrected analysis is more honest about which planner pathology was hitting prod. Tracking comment posted at #71197#issuecomment-4480624182.
...
## 2026-07-06 - HS-191718: Delta CI gate for notification coverage (rule seed / semantic path / preview) (PR #73598 draft)

**Type:** milestone
**Repository:** nutella
**Branch:** `kbachu-hs-191718-notification-coverage-ci-gate` (cut off `origin/main`)
**PR:** [#73598](https://github.com/highspot/nutella/pull/73598) (draft; base = `main`)
**Jira:** [HS-191718](https://highspot.atlassian.net/browse/HS-191718) (parent epic HS-183484 — Notifications CS1 Foundations)

**Files Changed:**
- `web/scripts/notifications-migration/scan_missing_email_previews.py` — rewritten (+564 / -202)
- `.buildkite/check_notification_coverage_delta.sh` — new (166 lines)
- `.buildkite/detect_changes.sh` — new `NOTIFICATION_REGISTRY` scoped change flag
- `.buildkite/pipeline.yml` — new gated step `Notification coverage delta gate`

**Summary:**
Wire a Buildkite CI gate that fails PRs which add a new notification kind to `AlertCommands::ALERT_CONFIG` or a real direct email type to `EmailCommands::SETTINGS` without also landing the three companion rows that make the notification functional end-to-end: (1) a rule seed in `NotificationRulesShape::SHEET_BACKED_ATTRIBUTES` (so the seed / realign migrations can hand the kind a `notification_rules` document with the right priority / product_category / batching_window), (2) a semantic production path (either a `*_KINDS` list under `common/email/semantic/builders/**`, an explicit `SemanticAlertRenderer.register(:<kind>, ...)` call, or a `DIRECT_BUILDERS` entry in `SemanticEmailRegistry`), and (3) a preview row in `IMMEDIATE_CATEGORIES` / `BATCHED_CATEGORIES` / `EMAIL_TYPE_PREVIEWS` in `semantic_email_preview.rb`. Delta-only: fails only on NEW misses this PR introduces; baseline gaps on `main` are tolerated so the gate can ship without a preflight cleanup PR. Same rollout shape as `sorbet_delta_zero.sh`.

**Changes Made:**
- **`scan_missing_email_previews.py` — three-surface scanner.** Refactored from a preview-only scanner into a unified coverage scanner over the three surfaces above. Encapsulated paths + parsing state in a `Scanner` class taking `project_root` (so the same script can run against a sibling git tree — used by the delta wrapper). Detection returns six missing-entry buckets: `missing_rule_seed_{alert,direct}`, `missing_production_{alert,direct}`, `missing_preview_{alert,direct}`.
- **Robustified Ruby parsing to fix false positives + false negatives seen during smoke tests.** (a) `_read` now strips whole-line comments (`^\s*#.*$`) before parsing, killing false positives where a commented-out `:sym => true` line inside `SETTINGS` was surfacing as a top-level entry. (b) `_hash_keys_in_body` handles both `:sym => val` and `sym: val` hash shorthand, closing the miss where the `SemanticEmailRegistry` `TRANSACTIONAL_TYPES` map used shorthand form and half of its entries were being counted as "unregistered". (c) `_symbols_from_pc_lists` handles `%i[]` / `%w[]` lists both at constant scope and inside hash literal values (`{ foo: %i[a b c] }`), so bulk-registration patterns in the builders are picked up. (d) `registered_alert_kinds` unions the `ALL_GENERIC_KINDS` constant with all `SemanticAlertRenderer.register(:<kind>` sites (both single-symbol and array-of-symbols forms).
- **CLI flags.** Added `--surfaces {rule,production,preview,all}` (defaults to `all`) so the delta wrapper can be reused for scoped audits, and `--project-root PATH` so the scanner can point at any git tree without symlink hacks.
- **`check_notification_coverage_delta.sh` — worktree-based delta wrapper.** Fetches the base branch (`origin/main` for PRs; parent commit as a fallback), determines merge-base, `git worktree add`s the merge-base into a scratch dir, runs the scanner (the HEAD version — not the base version, on purpose so scanner improvements land in one PR without a chicken-and-egg problem) against both trees, subtracts JSON to isolate new misses, and exits 1 with a Buildkite annotation naming each new missing kind under each surface. Cleanup runs on trap so the scratch worktree is removed on both PASS and FAIL.
- **`detect_changes.sh` — new `NOTIFICATION_REGISTRY` scoped change flag.** Fires on the two source-of-truth registries (`alert_commands.rb`, `email_commands.rb`), the three coverage surfaces (`notification_rules_shape.rb`, `semantic_email_registry.rb`, `semantic_email_preview.rb`), the builder tree (`web/common/email/semantic/builders/**`), and the scanner + delta script themselves. This scoping keeps the gate silent on unrelated PRs and self-testing on scanner changes.
- **`pipeline.yml` — new step `Notification coverage delta gate`.** Runs in a `python:3.11-slim` docker image (scanner is stdlib-only — no `pip install` step). Conditional on `HAS_NOTIFICATION_REGISTRY_CHANGES == "true"` or the `main` branch (so `main` picks up baseline drift too, even without a matching PR file change).

**Notes:**
- **Isolation strategy for the PR.** My active branch (`kbachu-hs-191017-semantic-builders-retention-bulk-pitch`) had 3 unrelated in-flight modifications from the retention_host_notification builder work. Rather than muddy that branch or risk stashing/reverting live changes, I `git worktree add`ed a scratch tree at `origin/main`, copied only my 4 files in, committed + pushed + opened the draft PR from there, then removed the worktree. The user's original branch and working tree are byte-identical to before the session started.
- **Pre-commit hooks passed cleanly on first attempt.** `branch-check`, `branch-name-check`, `install-dependencies`, `test-ownership`, `frozen-files`, `git-diff` all ran; all `-lint` / RuboCop / HAML / prettier / eslint hooks skipped (no matching file types). No `--no-verify` used.
- **Smoke tests before pushing.** (a) Happy path — unchanged tree → scanner reports zero drift → wrapper exits 0. (b) Fail path — injected a `ci_gate_smoketest_fake_kind` into `ALERT_CONFIG` (with no rule / builder / preview companions) → scanner correctly surfaced the delta on all three surfaces → wrapper exited 1 with a legible Buildkite annotation naming the fake kind under each surface. (c) Escape hatch — same fake kind with `:options => { :no_email => true }` → production + preview checks skipped, rule-seed check still applied (in-app channel still needs a seed regardless of email). Smoke-test residue reverted before the commit; grep for `ci_gate_smoketest` confirmed clean.
- **Debugging journey worth preserving.** (a) Initial fail-path run reported PASS — the smoke-test Python injection script was writing the fake kind past `ALERT_CONFIG`'s true closing brace because it was matching the wrong `}.freeze` marker. Fix: walk the brace structure from `ALERT_CONFIG = {` forward, counting `{` / `}` depth, to locate the actual close. (b) False positives on `settings_direct_email_types` (kinds like `account` surfacing as top-level entries) traced to commented-out `:sym => true` lines being parsed as hash entries — fixed by comment-stripping in `_read`. (c) `registered_direct_types` was reporting half of `TRANSACTIONAL_TYPES` as unregistered — traced to shorthand `sym: val` hash syntax that the initial `_hash_keys_in_body` regex only handled the `:sym => val` form. Extended the regex to handle both.
- **Rollout: delta-only, blocking from day one.** Baseline gaps on `main` at merge-base time (`skill_wgll_video_approved` etc. — most of which have since been added to `semantic_email_preview.rb` already) are tolerated. If the gate turns out too chatty in review, we can lower it to a soft-warning annotation by flipping the exit code without touching the scanner. Companion skill (`~/.cursor/skills/detect-missing-email-previews/SKILL.md`) is being updated in a follow-up commit to reflect the expanded scope.
